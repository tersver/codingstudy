...
#2.첫번째 : 대문자를 소문자로 바꾼다.
#3.두번째 : 특수문자를 제거한다
#4.세번째 : .이 2개이상 일때, 1개로 만든다.
#5.네번쨰 : 앞뒤가 .일 경우에 제거 한다.
#6.다섯번째 : 길이가 16 이상일때, 15자까지 자른다. 그 후, 맨 마지막 문자가 .일 경우 제거 한다.
#7.여섯번째 : 빈 문자열이 될 경우, "a"를 삽입한다.
#8.일곱번재 : 2문자 이하일 경우, 3문자가 될때까지 마지막 문자를 반복해서 더한다.
